...
Nt = 50
dt = tmax/Nt
times = [i*dt for i in range(Nt+2)]
...
# count = 0
for i in range(Nt+1): #Nt for Magnus, Nt+1 for regular time evolution
	# count +=1
	...
	#Magnus
	for j in range(i+1):
		step_h = [generate_ising(spins, (times[j+1]**2-times[j]**2)/(2*tmax)*V, dt*g)]
		step_h = np.append(step_h, dt**3/(6*tmax)*np.array([generate_magnus_2(spins,V,g)]))
		# step_h = np.flip(step_h)
		trotter = PauliTrotterEvolution(reps=1)
		initialize += np.sum([trotter.convert(step_h[j].exp_i()).to_circuit() for j in range(len(step_h))])
		# initialize += trotter.convert(step_h[0].exp_i()).to_circuit()
...
json.dump(log_data, open('data/trotter/Magnus2T1_Nt50.dat','w+'))
